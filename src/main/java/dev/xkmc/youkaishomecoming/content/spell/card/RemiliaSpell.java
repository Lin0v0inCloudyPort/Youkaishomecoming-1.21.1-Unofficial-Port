...
public class RemiliaSpell extends ActualSpellCard {
	...
	private void shootSpear(CardHolder holder, Vec3 target, Vec3 dir) {
		...
		for (int i = 0; i < n; i++) {
			...
			var e = holder.prepareDanmaku(30, dir.scale(3), DanmakuItems.Bullet.MENTOS, DyeColor.RED);
			e.setPos(pos);
			holder.shoot(e);
		}
	}
	...
	public static class Sweep extends Ticker<RemiliaSpell> {
		...
		public boolean tick(CardHolder holder, RemiliaSpell card) {
			...
			for (int i = 0; i < count; i++) {
				...
				var e = holder.prepareDanmaku(t, d.scale(v0), DanmakuItems.Bullet.BUBBLE, DyeColor.RED);
				holder.shoot(e);

				double mid = 0.6 + 0.3 * rand.nextDouble();
				e = holder.prepareDanmaku((int) (t / mid * 0.8), d.scale(v0 * mid), DanmakuItems.Bullet.MENTOS, DyeColor.RED);
				holder.shoot(e);

				double low = 0.3 + 0.3 * rand.nextDouble();
				e = holder.prepareDanmaku((int) (t / low * 0.6), d.scale(v0 * low), DanmakuItems.Bullet.BALL, DyeColor.RED);
				holder.shoot(e);
			}
	...
	public static class Lasers extends Ticker<RemiliaSpell> {
		...
		public boolean tick(CardHolder holder, RemiliaSpell card) {
			...
			for (int i = 0; i < count; i++) {
				...
				var l0 = holder.prepareLaser(life, cen, dir, len, DanmakuItems.Laser.LASER, DyeColor.LIGHT_BLUE);
				...
				for (int j = 0; j < spread; j++) {
					...
					var l1 = holder.prepareLaser(life, pos, dst, 80, DanmakuItems.Laser.LASER, DyeColor.LIGHT_BLUE);
					l1.setupTime(20, 10, life, 10);
					holder.shoot(l1);
				}
			}
			super.tick(holder, card);
